...
const ZfglPage = ({
    ...
    const pagination = {
        total: lszfgl.total,
        current: lszfgl.current,
        pageSize: lszfgl.size,
        onChange: (pageNo) => {
            onPageChange(pageNo)
        },
    ...
    function SizeChange(current, pageSize) {
        ...
        dispatch({type: 'lszfgl/updatePayload', payload: {size: pageSize, current: 1, offset: 0}});
        dispatch({type: 'lszfgl/query', payload: {}});
    ...
    function onPageChange(pageNo) {
        ...
        var offset = pageNo * lszfgl.size - lszfgl.size;
        dispatch({type: 'lszfgl/updatePayload', payload: {offset: offset, current: pageNo}});
        dispatch({type: 'lszfgl/query', payload: {}});
    ...
    function addReasonClick() {
        ...
        dispatch({type: 'lszfgl/updatePayload', payload: {modalVisible: true}});

    }

    function addZFClick() {

        dispatch({type: 'lszfgl/updatePayload', payload: {isAdd:true,modalVisible: true,currentSelectValue:0,currentSelectValue1:0,id:0,foodMethodAddItem:{}}});

    }4

    function onClear() {
        dispatch({type: 'lszfgl/updatePayload', payload: {keyword: ""}});
    }
    ...
    function onKeywordChange(e) {
        dispatch({type: 'lszfgl/updatePayload', payload: {keyword: e.target.value}});
    }

    const CpdwAddModalProps = {
        visible: lszfgl.modalVisible,
        dispatch,
        onOk() {

            dispatch({
                type: 'lszfgl/addFoodMethodUrl',
    ...
    const CpdwAddModalProps1 = {
        visible: lszfgl.modalEditVisible,
        dispatch,
        onOk() {

            dispatch({
                type: 'lszfgl/editFoodMethodUrl',
    ...
    return (
        <div className={styles.search}>
            ...
            <div style={{marginLeft: 20}}><span>做法名称</span><Input style={{marginLeft: 20, width: 220}}
                                                                  onChange={onKeywordChange} value={lszfgl.keyword} onPressEnter = {onButtonSearch}/>
            ...
            <Button style={{marginLeft: 10, marginTop: 20}} onClick={addZFClick}>新增做法</Button>

            <Table className={styles.table}
                   columns={columns}
                   dataSource={lszfgl.list}
