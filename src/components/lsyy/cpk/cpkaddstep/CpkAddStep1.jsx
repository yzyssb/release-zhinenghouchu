...
const CpkAddStep1 = ({
    ...
    function handleOk() {
        ...
        cpxx.nextData.map(function (item) {
        ...
        dispatch({type: 'cpxx/updatePayload', payload: {currentSteps: 1,sendNextData:sendNextData,pullAfter:pullToRestaurant}});
    }

    function cancel() {
        localStorage.setItem("selectedRightKeys","");
        dispatch({type: 'cpxx/updatePayload', payload: {modalStep1Visiable: false,currentSteps:0,rightTree:[],sendNextData:[],pullToRestaurant:[]}});
    }


    const modalOpts = {
        title: "选择总部菜品（直营店铺只可以选择总部允许售卖的菜品，且不可以修改）",
        visible: cpxx.modalStep1Visiable,
        onCancel: cancel,
        destroyOnClose:true,
        width: 600,
        maskClosable:false,
    ...
    function lastStep() {
        // localStorage.setItem("selectedRightKeys","");
        dispatch({type: 'cpxx/updatePayload', payload: {currentSteps: 0}})
            //rightTree:[],currentSteps: 0,sendNextData:[],pullToRestaurant:[]}});
        // dispatch({
    ...
    function getRadioValue(record,str) {
        var pullToRestaurantNew = cpxx.pullAfter;
                ...
                if (record.specId) {
                    for (var j = 0; j < pullToRestaurantNew[i].foodSpecs.length; j++) {
                        if (pullToRestaurantNew[i].foodSpecs[j].specId == record.specId) {
                            ...
                            dispatch({type: 'cpxx/updatePayload', payload: {pullAfter: pullToRestaurantNew}});
                            return;
                        }
                    }
                }
    ...
    function pullToRes() {
        if(cpxx.pullAfter.length == 1)
        {
            message.warning("请选择菜品");
            return;
        }
        var ls = cpxx.pullAfter;
        ls.splice(0,1)
        dispatch({type: 'cpxx/updatePayload', payload: {pullAfter:ls}});
        dispatch({
            type: 'cpxx/pullToRestaurant',
            payload: {}
        });
    ...
    return (
        <Modal {...cpxx.currentSteps == 0 ? modalOpts : modalOpts1} >
            <Steps style = {{width:'60%',margin:'0 auto'}} current={cpxx.currentSteps}>
                <Step title="选择菜品" description=""/>
                <Step title="选择价格" description=""/>
            </Steps>
            {<div style={{display:cpxx.currentSteps == 0 ? 'block' : 'none',marginTop: 30}}>
                <TreeTransfer
                    placeholder={'查找需要选择的菜品'}
                    showSearch={true}
                    treeWidth={250}
                    treeHeight={400}
                    getAllTreeData={getAllTreeData} leftTreeData={cpxx.newData} rightTreeData={cpxx.rightTree}
                    leftTitle={'分类 名称'}
                    rightTitle={'分类 名称'}/>
            ...
            {cpxx.currentSteps == 1 && <div style={{marginTop: 30}}>
                <Table
                    columns={columns}
                    pagination={false}
                    dataSource={cpxx.sendNextData}
...
export default Form.create()(CpkAddStep1);
