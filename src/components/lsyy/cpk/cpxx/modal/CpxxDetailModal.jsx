...
const TcxxDetailModal = ({
                   visible, onOk, onCancel, currentItem, dispatch, lscpxx,lscpfl,lstcxx,
    ...
    if (cpflList.length >0) {
        ...
        cpflList.map((a, b) => {
            ...
            children.push(
                ...
                style={{border: "1px solid #e8e8e8",borderBottom: "0",backgroundColor: a.id != lscpxx.selectCategoryId?"#ffffff":'#e5f9ff',textAlign: "center",lineHeight:"30px"}}>
                {a.name} </li>
            )
        })
    }
        ...
        function onDelete(index){
         ...
         var selectedList = lscpxx.recommendFoods;
            ...
            function managerPriceAndSpecHandle(record,index){
                ...
                let newPriceJson = record.priceJson.split(',')[0]/100 + '元,' + record.priceJson.split(',')[1]/100 + '元,' + record.priceJson.split(',')[2]/100 + '元,' + record.priceJson.split(',')[3]/100 + '元,' + record.priceJson.split(',')[4]/100 + '元'
                let newVipPriceJson = record.vipPriceJson.split(',')[0]/100 + '元,' + record.vipPriceJson.split(',')[1]/100 + '元,' + record.vipPriceJson.split(',')[2]/100 + '元,' + record.vipPriceJson.split(',')[3]/100 + '元,' + record.vipPriceJson.split(',')[4]/100 + '元'
    ...
    const columns = [
     ...
     const rowSelection = {
       ...
       selectedRowKeys:lscpxx.selectedRowKeys,
       onChange: onSelectChange,
    };

     function onSelectChange(selectedRowKeys,selectedRows){
       dispatch({type: 'lscpxx/updatePayload',payload:{selectedRowKeys:selectedRowKeys,selectedRows:selectedRows}});
    }
    ...
    function onAddClick(){
      ...
      var selectedList = lscpxx.recommendFoods;
       ...
       lscpxx.selectedRows.map((a)=>{

           lscpxx.recommendFoods.map((b)=>{
       ...
       if (selectedList.length >=5){
           ...
           selectedList.map((i,j)=>{
            ...
            item.originReduce = 0;
            item.vipPriceJson = i.vipPriceJson;
            item.vipReduce = 0;
            item.specName = i.specName;
            item.specId = i.specId;
            item.maxCount = i.maxCount*1;
      ...
      dispatch({type: 'lscpxx/updatePayload',payload:{recommendFoods:newSelectedList,selectedRowKeys:[]}});
    ...
    function secondHandleNext(){
             ...
             dispatch({type: 'lscpxx/updatePayload',payload:{lscpxxDetailVisible:false,selectedRowKeys:[]}});

        }

        function secondHandlePrevious(){
          
             dispatch({type: 'lscpxx/updatePayload',payload:{lscpxxDetailVisible:false}});

        }

         function onLeftClick(a){

             dispatch({type: 'lscpxx/updatePayload',payload:{selectCategoryId:a.id}});
             dispatch({type: 'lscpxx/queryFoodByCategory',payload:{selectCategoryId:a.id,id:lscpxx.foodId}});

        }

        function onSearchKeyWord(value){

            dispatch({ type: 'lscpxx/queryFoodByCategory', payload: {id: lscpxx.foodId,selectCategoryId:0,name:value} });
    ...
    const modalOpts = {
        title:'编辑推荐菜',
        visible:lscpxx.lscpxxDetailVisible,
        onOk:handleOk,
             ...
             <div style={{overflow: 'hidden',height:'80%'}}>
                 <div style={{width:'40%', float:'left'}}>
                    ...
                    />
                     ...
                     <ul style={{width:'19%', float:'left',overflow:'hidden',listStyle: "none",padding: "0",borderBottom: "1px solid #e8e8e8"}}>
                      ...
                      <Table className={styles.lscpxxDetailTable} style={{width:'80%', float:'right',height:'380px',overflowY:'auto'}}
                        columns={columns}
                        dataSource={foodList}
                ...
                <Button type="primary" style={{width:'8%', float:'left',padding:'0',position: 'absolute',top: '50%',left: '41%'}} onClick = {onAddClick}>添加</Button>
                 <div style={{width:'50%', float:'right',height:'80%'}}>
                    ...
                    <Table className={styles.lscpxxDetailTable} style={{height:'500px',overflowY:'auto',marginTop:10}}
