...
const TcxxDetailModal = ({
    ...
    if (cpflList.length >0) {
        ...
        cpflList.map((a, b) => {
            ...
            children.push(
                ...
                style={{border: "1px solid #e8e8e8",borderBottom: "0",backgroundColor: a.id != tcxx.selectCategoryId?"#ffffff":'#e5f9ff',textAlign: "center",lineHeight:"30px"}}>
                {a.name} </li>
            )
        })
    }
      ...
      function onPriceDiffInput(e,index){
          ...
          if (isNaN(e*100)){
              tcxx.selectFoodList[index].priceDiff = 0;
          } else{
      ...
      function managerPriceDiffHandle(record,index){
            ...
            if(record.orderNo){
               ...
               handlebtn.push(<span key={index}> {(record.priceDiff==null)?0:record.priceDiff/100}</span>)
                ...
                handlebtn.push(<span key={index} >
                     <InputNumber className={styles.inputnumber} min={0} max={Infinity} step={0.01} formatter={onChangeValueFormatter} onChange = {(e)=>onPriceDiffInput(e,index)} value = {(record.priceDiff==null || isNaN(record.priceDiff))?0:record.priceDiff/100}/>
                  </span>)
              }
    ...
    function secondHandleNext(){
             ...
             dispatch({type: 'tcxx/updatePayload',payload:{tcxxDetailVisible:false,selectedRowKeys:[]}});

        }

        function secondHandlePrevious(){
          
             dispatch({type: 'tcxx/updatePayload',payload:{tcxxDetailVisible:false}});

        }

         function onLeftClick(a){

             dispatch({type: 'tcxx/updatePayload',payload:{selectCategoryId:a.id}});
             dispatch({type: 'tcxx/queryFoodByCategory',payload:{id:a.id,selectCategoryId:a.id}});
    ...
    const modalOpts = {
        ...
        visible:tcxx.tcxxDetailVisible,
        onOk:handleOk,
