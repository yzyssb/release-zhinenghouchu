...
function CpbjAddPage({ menu, cpbjPageConfig,cpbjSetting, dispatch }) {
  ...
  function add(text, record,index){
  	var dataSource=cpbjPageConfig.dataSource,parentIndex=cpbjPageConfig.queryIndex
  	console.log(+dataSource[parentIndex].sub[dataSource[parentIndex].sub.length-1].key+1)
  	dataSource.push({
  		key: +dataSource[parentIndex].sub[dataSource[parentIndex].sub.length-1].key+1,
  		duration:'全周',
        start_tm: '00:00',
        end_tm:'24:00',
        name: '',
  ...
  function del(text, record,index){
  	var dataSource=cpbjPageConfig.dataSource,parentIndex=cpbjPageConfig.queryIndex
  	dataSource[parentIndex].sub.splice(index,1)
  	dispatch({
        type: 'cpbjPageConfig/updatePayload',
        payload: { dataSource: dataSource }
    });
  ...
  return(
    <Header {...HeaderProps}>
        <div>
          ...
          <Table
            bordered
            dataSource={cpbjPageConfig.dataSource[cpbjPageConfig.queryIndex].sub} 
            columns={columns} 
          />
          <div>
