...
const CpfjfEditDetail = ({
  ...
  if (cpfjfEditConfig.activity_starttime == "" && cpfjfEditConfig.activity_endtime == "") {
    rangPickerinitialValue = ""
  } else {
    rangPickerinitialValue = [moment(cpfjfEditConfig.activity_starttime, 'YYYY-MM-DD HH:mm:ss'), moment(cpfjfEditConfig.activity_endtime, 'YYYY-MM-DD HH:mm:ss')];
  }
  ...
  function storeHandleOk(data) {
    ...
    if (cpfjfEditConfig.storeList.length > 0 && cpfjfEditConfig.storeSelectedRowKeys.length > 0) {
      cpfjfEditConfig.storeList.map(function (item, index) {
        for (var i = 0; i < cpfjfEditConfig.storeSelectedRowKeys.length; i++) {
          if (item.id == cpfjfEditConfig.storeSelectedRowKeys[i]) {
            storeids += item.name + ","
          }
        }
      })
    ...
    dispatch({
      ...
      payload: {
        ...
        storeInitSelectedRowKeys: cpfjfEditConfig.storeSelectedRowKeys,
        storeids: storeids,
        // isResetForm:false
      }
    })
  ...
  function storeHandleCancel() {
    dispatch({
      ...
      payload: {
        storeModalVisible: false,
        storeSelectedRowKeys: cpfjfEditConfig.storeInitSelectedRowKeys  //点击取消让选中的值回到初始的状态点击取消清空选中的
      }
    })
  }
  ...
  const storeRowSelection = {
    selectedRowKeys: cpfjfEditConfig.storeSelectedRowKeys,  //指定选中项的项,这个对应的不是数组的index,而是数据的id
    // 改变时把选中的值赋给
  ...
  function goodHandleOk(data) {
    ...
    if (cpfjfEditConfig.goodList.length > 0 && cpfjfEditConfig.goodSelectedRowKeys.length > 0) {
      cpfjfEditConfig.goodList.map(function (item, index) {
        for (var i = 0; i < cpfjfEditConfig.goodSelectedRowKeys.length; i++) {
          if (item.id == cpfjfEditConfig.goodSelectedRowKeys[i]) {
            goodids += item.name + ","
          }
        }
      })
    ...
    dispatch({
      ...
      payload: {
        ...
        goodInitSelectedRowKeys: cpfjfEditConfig.goodSelectedRowKeys,
        goodids: goodids,
        // isResetForm: true
      }
    })
  ...
  function goodHandleCancel() {
    dispatch({
      ...
      payload: {
        goodModalVisible: false,
        goodSelectedRowKeys: cpfjfEditConfig.goodInitSelectedRowKeys  //点击取消让选中的值回到初始的状态点击取消清空选中的
      }
    })
  }
  ...
  const goodRowSelection = {
    selectedRowKeys: cpfjfEditConfig.goodSelectedRowKeys,  //指定选中项的项,这个对应的不是数组的index,而是数据的id
    // 改变时把选中的值赋给
    onChange: function (selectedRowKeys, selectedRows) {
      // console.log(selectedRowKeys)   //选中的key
      // console.log(selectedRows)  //选中的数据是哪一条
  ...
  return (
    ...
    <div>
      ...
      <div style={{ textAlign: 'left', marginBottom: "20px", background: "#f8f8f8", lineHeight: "40px", marginTop: "20px", fontWeight: 700, textIndent: "10px" }}>积分基本设置</div>
      ...
      <Form style={{ textAlign: "center" }}>
        ...
        >
          {getFieldDecorator('activity_name', {
            initialValue: cpfjfEditConfig.currentDataDetail.activityName,
            rules: [{
        ...
        <Row style={{ marginBottom: "10px" }}>
          ...
          <Col span="8" >
            <RadioGroup style={{ width: "100%", textAlign: "left" }} value={cpfjfEditConfig.timeRadio} onChange={changeTimeType}>
        ...
        {cpfjfEditConfig.timeRadio == 1 &&
          <Form.Item
            {...formItemLayout}
            label=" "
          >
            {getFieldDecorator('in_day', {
              initialValue: cpfjfEditConfig.in_day,
              rules: [{
          ...
          // <div>
            ...
            >
              {getFieldDecorator('in_week', {
                initialValue: cpfjfEditConfig.in_week,
                rules: [{
                  required: true, message: '请选择固定星期几',
                }],
              })(
                <CheckboxGroup style={{ textAlign: "left" }} options={options} />
              )}
            </Form.Item>
        ...
        {cpfjfEditConfig.timeRadio == 3 &&
          ...
          >
            {getFieldDecorator('range-time-picker',
              ...
              <RangePicker allowClear={false} showTime format="YYYY-MM-DD HH:mm:ss" onOk={onSearchDateChange} />
            )}
          </FormItem>
        }
        ...
        >
          ...
          {getFieldDecorator('storeids', {
            initialValue: cpfjfEditConfig.storeids,
        ...
        >
          ...
          {getFieldDecorator('goodids', {
            initialValue: cpfjfEditConfig.goodids,
        ...
        <div style={{ textAlign: 'left', marginBottom: "20px", background: "#f8f8f8", lineHeight: "40px", fontWeight: 700, textIndent: "10px" }}>会员权益设置</div>
        ...
        >
          {getFieldDecorator('member_take_in', {
            initialValue: cpfjfEditConfig.currentDataDetail.memberTakeIn,
            rules: [{
              required: true, message: '请选择是否参与活动',
            }],
          })(
            <RadioGroup style={{ marginLeft: 20 }} onChange={changeMemberTakeIn}>
        ...
        {/*会员参与活动时才展示input*/}
        {cpfjfEditConfig.memberTakeIn == 1 &&
          <FormItem {...formItemLayout}
            label="积分返还比例"
            style={{ textAlign: "left" }}
          >
            {getFieldDecorator('mem_count', {
              initialValue: cpfjfEditConfig.currentDataDetail.memCount,
              rules: [{
        ...
        <div style={{ textAlign: 'left', marginBottom: "20px", background: "#f8f8f8", lineHeight: "40px", fontWeight: 700, textIndent: "10px" }}>非会员权益设置</div>
        ...
        >
          {getFieldDecorator('dismember_take_in', {
            initialValue: cpfjfEditConfig.currentDataDetail.dismemberTakeIn,
            rules: [{
              required: true, message: '请选择是否参与活动',
            }],
          })(
            <RadioGroup style={{ marginLeft: 20 }} onChange={changeDismemberTakeIn}>
              <Radio key="a" value={1}>参与活动</Radio>
              <Radio key="b" value={2}>不参与</Radio>
            </RadioGroup>
          )}
        ...
        {/*会员参与活动时才展示input*/}
        {cpfjfEditConfig.dismemberTakeIn == 1 &&
          <FormItem {...formItemLayout}
            label="积分返还比例"
            style={{ textAlign: "left" }}
          >
            {getFieldDecorator('dis_count', {
              initialValue: cpfjfEditConfig.currentDataDetail.disCount,
              rules: [{
                required: true, message: '请输入积分返还比例',
              }, { pattern: /^([1-9]\d?|100)$/, message: "请输入1-100的整数！" }],
            })(
        ...
        <FormItem style={{ textAlign: 'center' }}>
          ...
          <Button style={{ margin: "0 20px" }} type="default" onClick={quxiao}>取消</Button>
        </FormItem>
      </Form >
      ...
      >
        <Table scroll={{ x: "100%", y: 380 }} rowKey={record => record.id} rowSelection={storeRowSelection} columns={storeColumns} dataSource={cpfjfEditConfig.storeList} pagination={false} />
      </Modal>
      ...
      <Modal
        title="选择产品"
        visible={cpfjfEditConfig.goodModalVisible}
        onOk={goodHandleOk}
        onCancel={goodHandleCancel}
      >
        <Table scroll={{ x: "100%", y: 380 }} rowKey={record => record.id} rowSelection={goodRowSelection} columns={goodColumns} dataSource={cpfjfEditConfig.goodList} pagination={false} />
      </Modal>
...
// 上面只是表单，把表单放到下面的页面中
function CpfjfEdit({ menu, dispatch, cpfjfEditConfig }) {
  ...
  return (
    <Header {...HeaderProps}>
      <CpfjfEditDetails dispatch={dispatch} cpfjfEditConfig={cpfjfEditConfig} />
    </Header>
  );
}
